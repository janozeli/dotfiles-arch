...
type WaveResult struct {
	...
	Tasks []units.TaskResult `json:"tasks"`
}
...
func NewManifest(dag *DAG, u []units.Unit) *Manifest {
	...
	for i, wave := range dag.Waves() {
		...
		for _, node := range wave {
			wr.Tasks = append(wr.Tasks, units.TaskResult{
				ID:     node.GlobalID,
				UnitID: node.UnitID,
				Status: "pending",
			})
...
func (m *Manifest) UpdateTask(globalID string, result units.TaskResult) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *Manifest) DeriveStatus(aborted bool) string {
	...
	for _, wave := range m.Waves {
		for _, task := range wave.Tasks {
			if task.Status == units.StatusError {
				hasError = true
			}
			if task.Status == units.StatusWarning {
				hasWarning = true
			}
		}
	}
